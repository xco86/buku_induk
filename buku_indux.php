...
/* Buku Induk Diubah Oleh Heru Subekti & M.Zaemakhrus */
...
<?php
    ...
    $reportgrid->setSQLColumn('b.biblio_id', 'b.title AS \''.__('Title').'\'','i.item_code AS \''.__('Item Code').'\'', 'COUNT(item_id) AS \''.__('Copies').'\'',
        'ma.author_name AS \''.__('Author').'\'',
        'pl.place_name AS \''.__('Publishing Place').'\'',
        'b.isbn_issn AS \''.__('ISBN/ISSN').'\'',
        'pb.publisher_name AS \''.__('Publisher').'\'',
        'b.publish_year AS \''.__('Publish Year').'\'',
        'b.edition AS \''.__('Edition').'\'',
        'b.collation AS \''.__('Collation').'\'',
        'b.language_id AS \''.__('Language').'\'',
        'b.call_number AS \''.__('Call Number').'\'',
        'b.input_date AS \''.__('Tgl.Input').'\'');
    // Urutan data pada buku induk pilih pada list di atas, ASC urutan a-z 1-9 dulu-sekarang , DESC urutan z-a 9-0 sekarang-dulu
    ...
    $subquery_str = '(SELECT DISTINCT bsub.biblio_id, bsub.gmd_id, bsub.title, bsub.isbn_issn, bsub.call_number, bsub.classification, bsub.language_id,
        bsub.publish_place_id, bsub.publish_year, bsub.edition, bsub.collation, bsub.publisher_id, bsub.input_date
        FROM biblio AS bsub
    ...
    $table_spec = $subquery_str.' AS b
        ...
        LEFT JOIN mst_author AS ma ON b.biblio_id = ma.author_id
    ...
    $xlsquery = 'SELECT b.biblio_id, b.input_date, b.title AS \''.__('Title').'\''.
        ...
        $_SESSION['tblout'] = "buku_induk";
    echo '<a href="../xlsoutput.php" class="btn btn-default">'.__('Export to spreadsheet format').'</a>';
